...
- La partie __U103 Server status :__ est simple test ping qui s'arrête au premier paquet __valide__ envoyé. Si le serveur répond, la réponse est __Serveur status :<span style=color:green>OK</span>__. Si le serveur ne répond pas, la réponse sera __Serveur status : <span style=color:RED>DOWN</span>__.
